...
theorem measure_iUnion_diameterThickening_le {mu : Measure E}
    (hmu : IsStraightMeasure mu) {alpha p : ℝ} (halpha : 0 < alpha) (hp : 0 ≤ p)
    {F : Set E} {chosen : Set (Set E)}
    (hchosen : chosen ⊆ badConvexSets mu F alpha) (hcountable : chosen.Countable) :
    mu (⋃ V : chosen, diameterThickening p (V : Set E)) ≤
      ENNReal.ofReal (2 * p + 1) *
        ∑' V : chosen, Metric.ediam (V : Set E) := by
  letI : Countable chosen := hcountable.to_subtype
  calc
    mu (⋃ V : chosen, diameterThickening p (V : Set E)) ≤
        ∑' V : chosen, mu (diameterThickening p (V : Set E)) :=
      measure_iUnion_le _
    _ ≤ ∑' V : chosen,
        Metric.ediam (diameterThickening p (V : Set E)) :=
      ENNReal.tsum_le_tsum fun V ↦
        hmu _ (isOpen_diameterThickening p (V : Set E)).measurableSet
    _ ≤ ∑' V : chosen,
        ENNReal.ofReal (2 * p + 1) * Metric.ediam (V : Set E) :=
      ENNReal.tsum_le_tsum fun V ↦
...
enlargement. -/
theorem exists_mem_not_mem_sevenDiameterThickening {mu : Measure E}
    (hmu : IsStraightMeasure mu) {F : Set E} (hF : MeasurableSet F)
    {alpha : ℝ} (halpha : 0 < alpha) {chosen : Set (Set E)}
    (hchosen : chosen ⊆ badConvexSets mu F alpha) (hcountable : chosen.Countable)
    (hdisjoint : chosen.PairwiseDisjoint id)
