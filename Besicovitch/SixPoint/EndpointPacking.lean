...
public import Besicovitch.SixPoint.BlueChildSwap
public import Besicovitch.SixPoint.EndpointFailureClosed
public import Besicovitch.SixPoint.FiniteProperty
public import Besicovitch.SixPoint.SiblingIncidenceClosed
...
bound for two ordered chords in the unit ball.
-/
...
/-- The weighted geometric inequality for two ordered sibling pairs in the unit ball of `E`. -/
def WeightedGeometricBound (E : Type*) [NormedAddCommGroup E] (lambda mu : ℝ) : Prop :=
  ∀ e p₁ p₂ w₁ w₂ : E,
    ‖e‖ = 1 →
    ‖p₁‖ ≤ 1 → ‖p₂‖ ≤ 1 → ‖w₁‖ ≤ 1 → ‖w₂‖ ≤ 1 →
    barC ≤ ‖p₁ - p₂‖ → barC ≤ ‖w₁ - w₂‖ →
...
private theorem weightedGeometricBound_configuration {lambda mu : ℝ}
    (hweighted : WeightedGeometricBound E lambda mu) {configuration : SixPointConfiguration E}
...
private theorem exists_nonnegative_score_of_selected_diagonal [InnerProductSpace ℝ E]
    {lambda mu : ℝ}
...
theorem sixPointFiniteProperty_barS_of_weightedGeometricBound [InnerProductSpace ℝ E]
    {lambda mu : ℝ}
    (hlambda : 0 < lambda) (hmu : 0 < mu)
    (hweighted : WeightedGeometricBound E lambda mu) : SixPointFiniteProperty E barS := by
