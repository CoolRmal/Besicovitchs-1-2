...
theorem IsStraightMeasure.exists_children {X : Type*} [MetricSpace X] [MeasurableSpace X]
    [OpensMeasurableSpace X] {μ : Measure X} (hμ : IsStraightMeasure μ) {e : Set X}
    (he : MeasurableSet e) {root : X} {d γ : ℝ}
